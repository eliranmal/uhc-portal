...
function setup_cloud_resources()
{
  if [[ ! -z $VPC_COUNT  &&  ! -z $REGION ]];then
    ...
    for((i=1; i <=$VPC_COUNT; i++)); do
        ...
        create_vpc="$(ocmqe create vpc --region ${REGION} --name ${VPC_NAME} 2>&1 | tail -n 3)"
        ...
        create_securitygroup="$(ocmqe create security-groups --name-prefix=${VPC_NAME} --region ${REGION} --vpc-id $vpc_id --count 2  2>&1 | tail -n 1)"
      ...
      # Loop through zones
        for zone in ${zones[@]} ; do
            ...
            create_linked_subnet="$(ocmqe create subnets --region ${REGION} --zones $zone --vpc-id  $vpc_id 2>&1 | tail -n 2)"
            ...
            public_subnet_name="${VPC_NAME}-public-$zone"
            private_subnet_name="${VPC_NAME}-private-$zone"
    ...
    if [ ! -z $DELETE_RESOURCES ];then
      ...
      for ZONE in $TOTAL_ZONES; do
        ...
        for((i=0; i <$TOTAL_VPC_PER_ZONE; i++)); do
          ...
          delete_vpc="$(ocmqe delete vpc --region ${ZONE} --vpc-id ${VPC_ID} --total-clean 2>&1)"
          echo $delete_vpc
          if [[ "$delete_vpc" =~ "Delete vpc success ${VPC_ID}" ]]; then
